...
const Map = ({setLoading}) => {
    ...
    useEffect(() => {
        if (!mapRef.current) {
            ...
            center: [0, 0],
            zoom: 5, // Start with a moderate zoom level
            minZoom: 2,
            maxZoom: 25, // Increase max zoom level
            scrollWheelZoom: true, 
            doubleClickZoom: true
            });
            
            const baseLayer = L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", {
                maxZoom: 25 // Ensuring tile layer supports deep zoom
    ...
    async function loadStressData() {
        try {
            ...
            onEachFeature: (feature, layer) => {
                const ndviValue = feature.properties.value;
                const stressLabel = ndviValue <= 0.3 ? "High Stress" : "No Stress";
                const stressColor = ndviValue <= 0.3 ? "red" : "green";
                
                            
                layer.bindPopup(`
                    <b>NDVI Value:</b> ${ndviValue.toFixed(3)}<br/>
                    <b>Unit:</b> ${feature.properties.unit || 'n/a'}<br/>
                    <b>Dataset:</b> Stress Sample ${datasetIndex + 1}<br/>
                    <b>Stress:</b> 
                    <span style="display: inline-block; width: 12px; height: 12px; background:${stressColor}; border:1px solid black; margin-right:5px;"></span>
                    ${stressLabel}
              `);
            }
          });
    ...
    return (
        ...
        <div style={{ 
            ...
            <div style={{ marginTop: "5px", fontSize: "0.8em", color: "#666" }}>
                Higher NDVI values (green) indicate healthy vegetation. <br/>
                Lower NDVI values (red) indicate stressed vegetation.
            </div>
            </div>
        </div>
        </div>
